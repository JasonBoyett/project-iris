...
const flashingGrid: React.FC<GridProps> = (props: GridProps) => {
    ...
    const step = () => {
        try {
            ...
            if (!index) {
                const newGrid: React.ReactElement[] =
                    grid as React.ReactElement[]
    ...
    useEffect(() => {
        const getWordsAndBuildGrid = async () => {
            const holder = await getWords(
                gridCount * cellCount * props.wordsPerCell,
            )
            setWords(
                partitionWords(
                    holder,
