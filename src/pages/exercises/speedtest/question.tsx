...
const Page: NextPage = () => {
  ...
  const handleClick = (answer: string) => {
    ...
    if(exerciseStore.current.responseCount === undefined) return
    if(!exerciseStore.current.responseCount === undefined) return
    console.log('ran')
    exerciseStore.setResponse(answer)
    exerciseStore.incrementResponseCount()
    if(exerciseStore.current.responseCount < TESTS_PER_DAY)
      router.replace('/exercises/speedtest')
        .catch((err) => console.log(err))
    else router.replace('/exercises/speedtest/result')
      .catch((err) => console.log(err))
  }
